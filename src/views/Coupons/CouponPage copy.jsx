...
const CouponsPage = () => {
  ...
  const stats = useMemo(() => {
    const active = coupons?.filter((c) => c.status === 'ACTIVE').length || 0;
    // Sum of all currentRedemptions to show real usage
    const totalRedeemed = coupons?.reduce((acc, curr) => acc + (curr.currentRedemptions || 0), 0) || 0;
  ...
  return (
    ...
    >
      <Container maxWidth="xl" sx={{ py: 4 }}>
        ...
        {/* KPI Section with modern elevation */}
...
const KPICard = ({ title, value, icon, color }) => (
  <Paper
    sx={{
      ...
      '&:hover': { transform: 'translateY(-5px)', boxShadow: `0 20px 25px -5px ${alpha(color, 0.1)}` }
    }}
  >
