...
const CouponsPage = () => {
  ...
  /**
   * Helper function to determine if a coupon is TRULY active
   * Logic: Must have isActive: true AND current date must be before validUntil
   */
  const checkIsLive = (coupon) => {
    ...
    // It's only LIVE if:
    // 1. Backend says it's active
    // 2. It hasn't expired yet (Current time < Expiry time)
    // 3. Manual isExpired flag is false
    return coupon.isActive === true && now < expiryDate && coupon.isExpired === false;
  };
  ...
  const filteredCoupons = useMemo(() => {
    ...
    return coupons.filter((coupon) => {
      const isLive = checkIsLive(coupon);

      if (tabValue === 0) return isLive; // Active Tab
      if (tabValue === 1) return !isLive; // Expired Tab
      return true; // All Tab
    });
  }, [coupons, tabValue]);
  ...
  const stats = useMemo(() => {
    ...
    const activeCount = coupons.filter((c) => checkIsLive(c)).length;
    const totalRedeemed = coupons.reduce((acc, curr) => acc + (curr?.currentRedemptions || 0), 0);

    return { active: activeCount, totalRedeemed };
  }, [coupons]);
